function [x_samples,time,AR,NP]=CARS(N_initial,N_desired_samples)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Cheap ARS (CARS)                %%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 U=rand(1,1);
     if U<eval_TAR/eval_PROP %%%% accept!!
      ...
      pos2=find(abs(S-xpr)==min(abs(S-xpr)));
      Saux=S;
      Saux(pos2)=xpr;
      Saux=sort(Saux);
      %%%% computing Area_tot of the alternative proposal construction
      Area2=Build_Prop(Saux,V,D_v);
      %%%% Update (or not) the set of nodes
          if Area2<=Area_tot(t)
             S=Saux;
          end
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
     end  %%%% end RS test and possible update of the set of nodes
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%         
